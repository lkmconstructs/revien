...
The rule classifier cannot recognize every sensitive claim — "I'm sober"
classifies as nothing. An unclassified claim might *be* a sensitive claim the
rules could not name. The hybrid backend must close this recognition gap so
sensitive claims are confidently classified and protected by the gate's own
mechanisms (protected-set, iron-grip), not only by the interim floor.

- This trigger fires **regardless of queue depth.** A comfortable queue does not
  retire it.
- Until it lands, the **interim sensitive floor** (below) holds the line.
